...
Imagine `Rc<T>` as a TV in a family room. When one person enters to watch TV, they turn it on. Others can come into the room and watch the TV. When the last person leaves the room, they turn off the TV because it's no longer being used. If someone turns off the TV while others are still watching it, there would be a problem.

We use the `Rc<T>` type when we want to allocate some data on the *heap* for multiple parts of our program to reaad and we can't determine at compile time which part will finish using the data last. If we knew which part would finish last, we could just make that part the data's owner, and the normal ownership rules enforced at compile time would take effect.

> Note that `Rc<T>` is only for use in single-threaded scenarios. When we discuss concurrency in [[Chapter 16 Fearless Concurrency]], we'll cover how to do reference counting in multi-threaded programs. 

## Using `Rc<T>` to Share Data

We'll return to the cons list example used earlier in this chapter. Recall that we defined it using `Box<T>`. This time, we'll create two lists that both share ownership of a third list. Conceptually, it'll look similar to this diagram:

![[Chapter15_Rc<T>Box_ShareDiagram.png]]
We'll create list `a` that contains `5` and then `10`. Then we'll make two more lists: `b` that start with `3` and `c` that starts with `4`. Both `b` and `c` lists will then continue on to the first `a` list containing `5` and `10`. In other words, both lists will share the first list containing `5` and `10`. 
...
We could change the definition of `Cons` hold references instead, but then we would have to specify lifetime parameters. By specifying lifetime parameters, we would be specifying that every element in the list will live at least as long as the entire list. This is the case for the elements and lists used in this example, but not in every scenario.
...
We could have called `a.clone()` rather than `Rc::clone(&a)`, but Rust's convention is use `Rc::clone` in this case. The implementation of `Rc::clone` doesn't make a deep copy of all the data like most types' implementations of `clone` do. The call to `Rc::clone` only increments the reference count, which doesn't take much time. Deep copies of data can take a lot of time. By using `Rc::clone` for reference counting, we can visually distinguish between the deep-copy kinds of clones and the kinds of clones that increase the reference count. When looking for performance problems in the code, we only need to consider the deep-copy clones and can disregard calls to `Rc:;clone`. 

## Cloning an `Rc<T>` Increases the Reference Count


Let's change our current working example so we can see the reference counts changing as we create and drop references to the `Rc<List>` in `a`. 

The following code, we'll change `main` so it has an inner scope around list `c`; then we can see how the reference count changes when `c` goes out of scope. 
...
At each point in the program where the reference count changes, we print the reference count, which we get by calling the `Rc::strong_count` function. This function is named `strong_count` rather than `count` because the `Rc<T>` type also has a `weak_count`; we'll see that in the "Preventing Reference Cycles: Turning an `Rc<T> into a Weak<T>`" section.
...
Via immutable references, `Rc<T>` allows you to share data between multiple parts of your program for reading only. If `Rc<T>` allowed you to have multiple mutable references too, you might violate one of the borrowing rules discussed in [[Chapter 4 Understanding Ownership]]: multiple mutable borrows to the same place can cause data races and inconsistencies. But being able to mutate data is very useful! 

The next section will discuss the interior mutability pattern and the `RefCell<T>` type that you can use in conjunction with an `Rc<T>` to work with this immutability restriction. 


# 15.5 `RefCell<T>` and the Interior Mutability Pattern

*Interior Mutability*:
- Design pattern in Rust that allows you to mutate data even when there are immutable references to that data
	- Normally disallowed due to the borrowing rules
- To mutate data, the pattern uses `unsafe` code inside a data structure to bend Rust's usual rules that govern mutation and borrowing
- Unsafe code tells the compiler that we're checking the rules manually instead of relying on the compiler to check them for us
	- Will be discussed more in [[Chapter 19 Advanced Features]]
...
We'll explore this concept using the `RefCell<T>` type that follows the interior mutability pattern.

## Enforcing Borrowing Rules at Runtime with `RefCell<T>`

Unlike `Rc<T>`, the `RefCell<T>` type represents single ownership over the data it holds. So what makes `RefCell<T>` different from a type like `Box<T>`? Recall the borrowing rules from [[Chapter 4 Understanding Ownership]]: 
- At any given time, you can have *either* (but not both) one mutable reference or any number of immutable references.
- References must always be valid

With references and `Box<T>`, the borrowing rules' invariants are enforced at compile time. With `RefCell<T>`, these invariants are enforced at *runtime*. With references, if you break these rules you'll get a compiler error. With `RefCell<T>`, your program will panic and exit instead.
...
Advantage of checking borrowing rules at runtime instead is that certain memory-safe scenarios are then allowed, where they would've been disallowed by the compile-time check. Static analysis, like the Rust compiler, is inherently conservative. Some properties of code are impossible to detect by analyzing the code: the most famous example is the Halting Problem, which is beyond the scope of this book but is interesting to research about. 

Because some analysis is impossible, the Rust compiler can't be sure the code complies with the ownership rules, it might reject a correct program for example. The `RefCell<T>` type is useful when you're sure your code follows the borrowing rules but the compiler is unable to understand and guarantee that.

Similar to `Rc<T>`, `RefCell<T>` is only for use in single-threaded scenarios and will you give a compile-time error if you try using it in a multi-threaded context. We'll talk about to get the functionality of `RefCell<T>` in a multi-threaded program in [[Chapter 16 Fearless Concurrency]]. 
...
The following will be a practical example of where we can use `RefCell<T>` to mutate an immutable value and see why it is useful.
...
Rust doesn't have objects in the same sense as other languages have objects, and Rust doesn't have mock object functionality built into the standard library as other languages do. However, you can definitely create a struct that will serve the same purpose.

Scenario: We'll create a library that tracks a value against a maximum value and sends messages based on how close to the maximum value the current value is.

Our library will only provide the functionality of tracking how close to the maximum a value is and what the messages should be at what times. Applications that use our library will be expected to provide the mechanism for sending the messages: the application could put a message in the application, send an (email, text message, etc). Library doesn't need to know that details. All it needs is something that implements a trait we'll provide called `Messenger`. 
...
- `Messenger` trait has one method called `send` that takes an immutable reference to `self` and the text of the message. 
	- This trait is the interface our mock objects needs to implement so that the mock can be used in the same way a real object is. 
- We want to test the behavior of `set_value` method on `LimitTracker`
...
- We need a mock object that, instead of sending an email or text message when we call `send`, will only keep track of the messages it's told to send. 
	- We can create a new instance of the mock object, create a `LimitTracker` that uses the mock object, calls the `set_value` method on `LimitTracker`, and then check that the mock object has the messages we expect
 The following code will show how to implement that test
...
Explaining the Code;
- Test code defines a `MockMessenger` struct that has a `sent_messages` filed with a `Vec` of `String` values to keep track of the messages it's told to send. 
- We define an associated function `new` to make it convenient to create new `MockMessenger` values that start with an empty list of messages
- We then implement the `Messenger` trait for `MockMessenger` so that we can give a `MockMessenger` to `LimitTracker`.
...
We're testing what happens when the `LimitTracker` is told to set `value` to something that is more than 75 percent of the `max` value. 
1) Create a new `MockMessenger`, which will start with an empty list of message. 
2) Create a new `LimitTracker` and give it a reference to the new `MockMessenger` and a `max` value of 100
3) Call the `set_value` method on the `LimitTracker` with a value of 80, which is more than 75% of 100.
4) Assert that the list of messages that the `MockMessenger` is keeping track of should now have one message in it.

However, there is one problem with this test as shown below:
```shell
$ cargo test
...
This is a situation where interior mutability can help. We'll store the `sent_messages` within a `RefCell<T>` and then the `send` method will be able to modify the `send_messages` to store the messages we've seen.
...
- `sent_messages` field is now of type `RefCell<Vec<String>>` instead of `Vec<String>`.
	- In the new function, we create a new `RefCell<Vec<String>>` instance around the empty vector
- Implementation of the `send` method, the first parameter is still an *immutable borrow* of `self`, which matches the trait definition.
- Call `borrow_mut` on the `RefCell<Vec<String>>` in `self.sent_messages` to get a mutable reference to the value inside the `RefCell<Vec<String>>` which is the vector
	- We can call `push` on the mutable reference to the vector to keep track of the messages sent during the test
- Last change is in the assertion
	- To see how many items are in the inner vector, we call `borrow` on the `RefCell<Vec<String>>` to get an immutable reference to the vector


## Keeping Track of Borrows with `RefCell<T>`

When creating immutable and mutable references, we use the `&` and `&mut` syntax. With `RefCell<T>`, we use the `borrow` and `borrow_mut` methods, which are part of the safe API that belongs to `RefCell<T>`. The `borrow` method returns the smart pointer type `Ref<T>`, and `borrow_mut` returns the smart pointer type `RefMut<T>`. Both types implement `Deref`, so we can treat them like regular references. 

The `RefCell<T>` keeps track of how many `Ref<T>` and `RefMut<T>` smart pointers are currently active. Every time we call `borrow`, the `RefCell<T>` increases its count of how many immutable borrows are active. When a `Ref<T>` goes out of scope, the count of immutable borrows down by one, just like the compile-time borrowing rules. `RefCell<T>` lets us have many immutable borrows or one mutable borrow at any point in time. 

If we try to violate these rules, the implementation of `RefCell<T>` will panic at runtime. The following code shows a modification of the `send` method above. We're deliberately trying to create two mutable borrows active for the same scope to show `RefCell<T>` prevents us from doing so at runtime. 
...
- Create a variable `one_borrow` for the `RefMut<T>` smart pointer returned from `borrow_mut`.
- Create another mutable borrow in the same way in the variable `two_borrow`
	- This created two mutable references in the same scope which isn't allowed.

The code will compile but the test will fail:
```shell
$ cargo test
   Compiling limit-tracker v0.1.0 (file:///projects/limit-tracker)
...
Notice that the code panicked with the message `already borrowed: BorrowError`. This is how `RefCell<T>` handles violations of the borrowing rules at runtime.

Choosing to catch borrowing errors at runtime rather than compile time, means you'd potentially be finding mistakes in your code later in the developmental process; possibly not **until your code was deployed to production**. Also, your code would incur a small runtime performance penalty as a result of keeping track of the borrows at runtime rather than compile time. However, using `RefCell<T>` makes it possible to write a mock object that can modify itself to keep track of the messages it has seen while you're using it in a context where only immutable values are allowed. You can use `RefCell<T>` despite its trade-offs to get more functionality than regular references provide. 

## Having Multiple Owners of Mutable Data by Combining `Rc<T>` and `RefCell<T>`

A common way to use `RefCel<T>` is in combination with `Rc<T>`. 
...
For example, in the `Cons list` example where we used `Rc<T>` to allow multiple lists to share ownership of another list. Because `Rc<T>` holds only immutable values, we can't change any of the values in the list once we've created them. Let's add in `RefCell<T>` to gain the ability to change the values in the lists. The below code shows that by using a `RefCell<T>` in the `Cons` definition, we can modify the value stored in all the lists:
...
- Create a `List` in `a` with a `Cons` variant that holds `value`. 
	- We need to clone `value` so that both `a` and `value` have ownership of the inner `5` value rather than transferring ownership from `value` to `a` or having `a` borrow from `value`. 
- Wrap the list `a` in an `Rc<T>` so when we create lists `b` and `c`, they can both refer to `a`.
- After we've created the lists in `a`, `b`, and `c`, we want to add 10 to the value in `value`. 
	- Do this by calling `borrow_mut` on `value`, which uses the automatic dereferencing feature we discussed in [[Chapter 5 Using Structs to Structure Related Data]] ("Where's the `->` Operator ") to dereference the `Rc<T>` to the inner `RefCell<T>` value.
...
When we print `a`, `b`, and `c`, we can see they all have  the modified value of 15 rather than 5:
```shell
 ~/P/i/c/multiple_owners   …  cargo run                  6.2s  Fri 14 Apr 2023 04:33:32 PM EDT
   Compiling multiple_owners v0.1.0 (/home/ziyan/Projects/intro_rust/chapter15/multiple_owners)
    Finished dev [unoptimized + debuginfo] target(s) in 0.18s
     Running `target/debug/multiple_owners`
...
This technique is pretty neat! By using `RefCell<T>`, we have an outwardly immutable `List` value. But we can use the methods on `RefCell<T>` that provide access to its interior mutability so we can modify our data when we need to. The runtime checks of the borrowing rules protect us from data races, and it’s sometimes worth trading a bit of speed for this flexibility in our data structures. Note that `RefCell<T>` does not work for multi-threaded code! `Mutex<T>` is the thread-safe version of `RefCell<T>` and we’ll discuss `Mutex<T>` in [[Chapter 16 Fearless Concurrency]].

# 15.6 Reference Cycles Can Leak Memory

Rust's memory safety guarantees make it difficult but not impossible to accidentally create memory that is never cleaned up (*memory leak*). Preventing memory leaks entirely is not one of Rust's guarantees, meaning memory leaks are memory safe in Rust. Rust allows memory leaks by using `Rc<T>` and `RefCell<T>` where you create references where items refer to each other in a cycle. This creates a memory leak because the reference count of each item in the cycle will never reach 0, and the values will never be cleared.

## Creating a Reference Cycle

Here's an example of how a reference cycle might happen, starting with the definition of the `List` enum and a `tail` method:
...
- We're using another variation of the `List` definition. 
- The second element in the `Cons` variant is now `RefCell<Rc<List>>`: 
	- Instead of having the ability to modify the `i32` value, we want to modify the `List` value a `Cons` variant is pointing to.
- Adding a `tail` method to make it convenient for us to access the second item if we have a `Cons` variant.

The following code, we're adding a `main` function that uses the definitions above. This code creates a list in `a`, and a list in `b` that points to the list in `a`. Then it modifies the list in `a` to point to `b`, creating a reference cycle. There are `println!` statements along the way to show what the reference counts are at various points in this process.
...
- Next, we create an `Rc<List>` instance holding another `List` value in the variable `b` that contains the value `10` and points to the list in `a`. 
- Modified `a` so it points to `b` instead of `Nil`, creating a cycle. 
	- Achieved by using the `tail` method to get a reference to the `RefCell<Rc<List>>` in `a` which we put in the variable `link`. 
...
Running the command without the last `println!` outputs the following:
```shell
 ~/P/i/c/reference_leak   …  cargo run                      7.6s  Sat 15 Apr 2023 12:03:06 PM EDT
   Compiling reference_leak v0.1.0 (/home/ziyan/Projects/intro_rust/chapter15/reference_leak)
    Finished dev [unoptimized + debuginfo] target(s) in 0.10s
     Running `target/debug/reference_leak`
...
The reference count of the `Rc<List>` instances in both `a` and `b` are 2 after we change the list in `a` to point to `b`. At the end of `main`, Rust drops the variable `b` which decreases the reference count of the `b` `Rc<List>` instance from 2 to 1. The memory that `Rc<List>` has on the heap won't be dropped because its reference count is 1, not 0. Then Rust drops `a`, which decreases the reference count of the `a Rc<List>` from 2 to 1 as well. The memory allocated to the list will remain uncollected forever.

![[Reference_Count_MemoryLeak.png]]

Uncommenting the last `prinln!` will cause Rust to try to print this cycle until it overflows the stack. 

Creating a reference cycle is not easily done, but it's not impossible. If you have `RefCell<T>` values that contain `Rc<T>` value or similar nested combinations of types with interior mutability and reference counting, you must ensure that you don't create cycles. Creating a reference cycle is a logic bug and you should use automated tests, code reviews, and other best developmental practices to minimize. 

Another solution for avoiding reference cycles is reorganizing your data structures so that some references express ownership and some references don't. As a result, you can have cycles made up of some ownership relationships and some non-ownership relationships, and only the ownership relationships affect whether or not a value can be dropped. In the above code, we always want `Cons` variants to own their list, so reorganizing the data structure isn't possible. 

Our next example will be looking at graphs made up of parent nodes and child nodes to see when non-ownership relationships are an appropriate way to prevent reference cycles. 
...
Because the value that `Weak<T>` reference might have been dropped, to do anything with the value that a `Weak<T>` is pointing to, you must make sure the value still exists. Do this by calling the `upgrade` method on a `Weak<T>` instance, which will return an `Option<Rc<T>>`. 
You'll get a result of `Some` if the `Rc<T>` value has not been dropped yet and a result of `None` if the `Rc<T>` value has been dropped. Since `upgrade` returns an `Option<Rc<T>>`, Rust will ensure that the `Some` case and the `None` case are handled.

As an example, instead of using a list whose items know only about the next item, we'll create a tree whose items know about their children items *and* their parent items.

## Creating a Tree Data Structure: a `Node` with Child Nodes

We'll build a tree with nodes that know about their child nodes. We'll create a struct named `Node` that holds its own `i32` value as well as references to its children `Node` values:
...
- The node should own its children, and we want to share that ownership with variables so we can access each `Node` int he tree directly. 
	- Do this by defining the `Vec<T>` items to be values of type `Rc<Node>`
	- We want to modify which nodes are children of another node, so we have a `RefCell<T>` in `children` around the `Vec<Rc<Node>>`. 

We'll use our struct definition and create one `Node` instance named `leaf` with the value 3 and no children and another instance named `branch` with the value 5 and `leaf` as one of its children as shown below. 
...
We clone the `Rc<Node>` in `leaf` and store that in `branch`, meaning the `Node` in `leaf` now has two owners: `leaf` and `branch`. We can get from `branch` to `leaf` through `branch.children`, but there's no way to from `leaf` to `branch`. The reason is that `leaf` has no reference to `branch` and doesn't know they're related. We need to make `leaf` know that `branch` is its parent.

### Adding a Reference from a Child to its Parent

To make the child node aware of its parent, we need to add a `parent` field to our `Node` struct definition. The trouble is in deciding what the type of `parent` should be. We know it can't contain `Rc<T>` because that would create a reference cycle with `leaf.parent` pointing to `branch` and `branch.children` pointing to `leaf`.

Thinking about relationships another way, a parent node should own its children: if a parent node is dropped, its child nodes should be dropped as well. However a child should not own its parent: if we drop a child node, the parent should still exist --> weak reference

So instead of `Rc<T>`, we'll make the type of `parent` use `Weak<T>`, specifically a `RefCell<Weak<Node>>`.

The `Node` struct will look like the following:
```rust
use std::cell::RefCell;
use std::rc::{Rc, Weak};
...
A node will be able to refer to its parent node but doesn't own its parent. We update the `main` to use this new definition so the `leaf` node will have a way to refer to its parent, `branch`:
```rust
fn main() {
    let leaf = Rc::new(Node {
        value: 3,
...
- Creating a `leaf` node looks similar to the previous version with the exception of the `parent` field
	- `leaf` starts out without parent, so we create a new, empty `Weak<Node>` reference instance.
...
When we create the `branch` node, it will also have a new `Weak<Node>` reference in the parent field, because `branch` doesn't have a parent node. We still have `leaf` as one of the children of `branch`. Once we have the `Node` instance in `branch`, we can modify `leaf` to give it a `Weak<Node>` reference to its parent. We use the `borrow_mut` method on the `RefCell<WSeak<Node>>` in the `parent` field of `leaf`, and then we use the `Rc::downgrade` function to create a `Weak<Node>` reference to `brnach` from the `Rc<Node>` in `branch`.

When we print the parents of `leaf` again, this time we'll get a `Some` variant holding `branch`; now `leawf `can access its parent. When we print `leaf` we also avoid the cycle that eventually ended in a stack overflow.
...
- After `leaf` is created, its `Rc<Node>` has a strong count of 1 and a weak count of 0
	- In the *inner scope*, we create a `branch` and associate it with `leaf` -> the `Rc<Node` in `branch` will have a strong count of 1 and a weak count of 1 (for `leaf.parent` pointing to `branch` with a `Weak<Node>`)
- When we print the counts in `leaf`, we'll see it will have a strong count of 2, because `branch` now has a clone of the `Rc<Node>` of `leaf` stored in `branch.children`, but it will still have a weak count of 0.
- When the *inner scope* wends, `branch` goes out of scope and the strong count of the `Rc<Node>` decreases to 0, so its `Node` is dropped. 
